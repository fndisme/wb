...
namespace WebGame
{
  namespace XSSocketTraits
  {
    ...
    */
    ...
    template<typename D, typename Fun, typename Strand>
      void absorbAndDispatchMessage(socket_t& socket, const Fun& fun, Strand& strand)
      {
        while(true) {
          message_t msg ;
          ...
          std::shared_ptr<D> db = make_shared<D>();
          bool ok = db->import_from_array(static_cast<const void*>(msg.data()), msg.size()) ;

          if(ok) {
            strand.dispatch(boost::bind(fun, db));
          } else {
